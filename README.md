...
検出判定に該当したユーザーの動作は設定で変更できます。`自動ポップアップ`は検出時に履歴ポップアップを自動で開く動作です。閉じたユーザーは、その配信中は再表示されません。
...
The action after a detection match is configurable. `Auto popup` opens the history popup automatically when a match is detected. If you close that popup, it will not be auto-shown again during the same stream.
